...
public class ShapeCpu implements Serializable, Shape {
    ...
    public int getIndex(int... indices) {
        if (indices.length != dimension.length) {
            throw new IllegalArgumentException(String.format("索引维度(%d)与形状维度(%d)不匹配", indices.length, dimension.length));
        }
        ...
        for (int i = 0; i < indices.length; i++) {
            ...
            if (indices[i] < 0 || indices[i] >= dimension[i]) {
                throw new IndexOutOfBoundsException(String.format("索引[%d]=%d超出范围[0,%d)", i, indices[i], dimension[i]));
            }
    ...
    public int getDimension(int dimIndex) {
        if (dimIndex < 0 || dimIndex >= dimension.length) {
            throw new IndexOutOfBoundsException(String.format("维度索引%d超出范围[0,%d)", dimIndex, dimension.length));
        }
